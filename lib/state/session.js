 ...
 *
 * This is the state store implementation for the sso-server used when
...
SessionStore.prototype.store = function(req, callback) {
    if (!req.session) { return callback(new Error('SSO Server requires session support when using state. Did you forget to use express-session middleware?')); }
